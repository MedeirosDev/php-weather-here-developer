<?php

namespace MedeirosDev\WeatherHereDeveloper\Entities;


/**
 * Class Language
 * @package MedeirosDev\WeatherHereDeveloper\Entities
 */
class Language
{
    const AFRIKAANS = 'afrikaans';
    const ALBANIAN = 'albanian';
    const AMHARIC = 'amharic';
    const ARABIC = 'arabic';
    const ASSAMESE = 'assamese';
    const AZERBAIJANI = 'azerbaijani';
    const BASQUE = 'basque';
    const BELARUSIAN = 'belarusian';
    const BENGALI_BD = 'bengali-bd';
    const BENGALI = 'bengali';
    const BOSNIAN = 'bosnian';
    const BULGARIAN = 'bulgarian';
    const CATALAN = 'catalan';
    const CHINESE_PRC = 'chinese_prc';
    const CHINESE_HK = 'chinese_hk';
    const CHINESE_TW = 'chinese_tw';
    const CROATIAN = 'croatian';
    const CZECH = 'czech';
    const DANISH = 'danish';
    const DUTCH = 'dutch';
    const ENGLISH = 'english';
    const ENGLISH_US = 'english_us';
    const ESTONIAN = 'estonian';
    const FARSI = 'farsi';
    const FARSI_AF = 'farsi-af';
    const FINNISH = 'finnish';
    const FRENCH_CA = 'french_ca';
    const FRENCH = 'french';
    const GALICIAN = 'galician';
    const GEORGIAN = 'georgian';
    const GERMAN = 'german';
    const GREEK = 'greek';
    const GUJARATI = 'gujarati';
    const HAUSA = 'hausa';
    const HEBREW = 'hebrew';
    const HINDI = 'hindi';
    const HUNGARIAN = 'hungarian';
    const ICELANDIC = 'icelandic';
    const IGBO = 'igbo';
    const INDONESIAN = 'indonesian';
    const ITALIAN = 'italian';
    const JAPANESE = 'japanese';
    const KANNADA = 'kannada';
    const KASHMIRI = 'kashmiri';
    const KAZAKH = 'kazakh';
    const KHMER = 'khmer';
    const KIRGHIZ = 'kirghiz';
    const KOREAN = 'korean';
    const LAO = 'lao';
    const LATVIAN = 'latvian';
    const LINGALA = 'lingala';
    const LITHUANIAN = 'lithuanian';
    const MACEDONIAN = 'macedonian';
    const MALAGASY = 'malagasy';
    const MALAY = 'malay';
    const MALAYALAM = 'malayalam';
    const MARATHI = 'marathi';
    const MONGOLIAN = 'mongolian';
    const NORWEGIAN = 'norwegian';
    const ORIYA = 'oriya';
    const POLISH = 'polish';
    const PORTUGUESE_BR = 'portuguese_br';
    const PORTUGUESE = 'portuguese';
    const PUNJABI = 'punjabi';
    const PUSHTO = 'pushto';
    const ROMANIAN = 'romanian';
    const RUSSIAN = 'russian';
    const SERBIAN = 'serbian';
    const SESOTHO = 'sesotho';
    const SINHALA = 'sinhala';
    const SLOVAK = 'slovak';
    const SLOVENE = 'slovene';
    const SPANISH = 'spanish';
    const SPANISH_AM = 'spanish_am';
    const SWAHILI = 'swahili';
    const SWEDISH = 'swedish';
    const TAGALOG = 'tagalog';
    const TAMIL = 'tamil';
    const TELUGU = 'telugu';
    const THAI = 'thai';
    const TAJIK = 'tajik';
    const TURKISH = 'turkish';
    const TURKMEN_LATIN = 'turkmen_latin';
    const UKRAINIAN = 'ukrainian';
    const URDU = 'urdu';
    const UZBEK = 'uzbek';
    const VIETNAMESE = 'vietnamese';
    const XHOSA = 'xhosa';
    const YORUBA = 'yoruba';
    const ZULU = 'zulu';
}